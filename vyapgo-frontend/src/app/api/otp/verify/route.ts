...
/**
 * ✅ Next.js-15-safe route: only async export, no top-level Firebase.
 */
export async function POST(req: Request) {
  if (isBuildPhase) {
    console.log("⚙️ Skipping OTP verify route during Vercel build");
    return NextResponse.json(
      { ok: true, message: "Skipped during build" },
      { status: 200 }
    );
  }
  ...
  try {
    ...
    if (!data)
      return NextResponse.json(
        { ok: false, message: "No OTP pending" },
        { status: 400 }
      );
    ...
    return NextResponse.json(
      { ok: false, message: e?.message || "OTP verify failed" },
      { status: 500 }
    );
  }
}
