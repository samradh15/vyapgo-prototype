      ...
      >
        ...
        <div className="px-4 mt-1 flex items-center gap-8 text-sm">
          {(['inventory', 'sales', 'staff'] as Tab[]).map((t) => (
            <button
              key={t}
              className={`pb-2 capitalize ${tab === t ? 'text-white' : 'text-white/55'} relative transition-colors`}
              onClick={() => setTab(t)}
              aria-label={`Open ${t}`}
            >
...
export default function LoginUnified() {
  return (
    <div style={{ minHeight: '100vh', background: BG }} className="flex items-center justify-center p-6">
      <div
      ...
      >
        <div
          ...
          style={{ background: 'linear-gradient(to bottom, transparent, #F1E6D6 18%, #F1E6D6 82%, transparent)' }}
        />
        <section
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <Link href="/" className="text-[13px] text-gray-600 hover:text-gray-900 inline-flex items-center gap-2">
              ← Back to website
            </Link>
          </div>
          <div
